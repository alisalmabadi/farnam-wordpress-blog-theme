...
function marketpanel_siteoptions_add_admin() {
    ...
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'siteoptions' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			...
			marketpanel_of_reset_options($options,'siteoptions');
			header("Location: admin.php?page=siteoptions&reset=true");
			die;
		}
    }

    $mw_page = add_theme_page(__('تنظیمات پوسته','marketwplang'), __('تنظیمات پوسته','marketwplang'), 'edit_theme_options', 'siteoptions','marketpanel_siteoptions_options_page');
	add_action("admin_print_scripts-$mw_page", 'marketpanel_of_load_only');
	add_action("admin_print_styles-$mw_page",'marketpanel_of_style_only');
} 

add_action('admin_menu', 'marketpanel_siteoptions_add_admin');
...
function marketpanel_of_reset_options($options,$page = ''){
...
function marketpanel_siteoptions_options_page(){
    $options =  get_option('of_template');
?>

<div class="wrap" id="truethemes_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save">تنظیمات به روزرسانی شدند.</div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">تنظیمات بازنشانی شدند.</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    ...
    <?php 
        ...
        $return = marketpanel_siteoptions_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
      <img style="margin-top:5px;margin-bottom:5px;" src="<?php bloginfo(template_url);?>/admin/images/logo.png">
        <ul>
          <?php echo $return[1] ?>
        </ul>
      </div>
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    ...
    <input name="reset" type="submit" value="بازگردانی تنظیمات" class="button submit-button reset-button" onclick="return confirm('CAUTION: Any and all settings will be lost! Click OK to reset.');" />
...
/*-----------------------------------------------------------------------------------*/
/* بارگذاری استایل های لازم برای پنل تنظیمات
/*-----------------------------------------------------------------------------------*/

function marketpanel_of_style_only() {
...
/*-----------------------------------------------------------------------------------*/
/* بارگذاری فایل های جاوا اسکریپت لازم برای پنل تنظیمات
/*-----------------------------------------------------------------------------------*/

function marketpanel_of_load_only() {

	add_action('admin_head', 'marketpanel_of_admin_head');
	...
	function marketpanel_of_admin_head() { 
	?>
...
add_action('wp_ajax_of_ajax_post_action', 'marketpanel_of_ajax_callback');

function marketpanel_of_ajax_callback() {
...
function marketpanel_siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "multicheck":
			...
			$mw_key = $value['id'] . '_' . $key;
			$saved_std = get_option($mw_key);
			...
			elseif( $std == $key) {
			   $checked = 'checked="checked"';
				...
				$checked = '';                                                                                    }
			$output .= '<input type="checkbox" class="checkbox of-input" name="'. $mw_key .'" id="'. $mw_key .'" value="true" '. $checked .' /><label for="'. $mw_key .'">'. $option .'</label><br />';
		...
		case "upload":
			...
			$output .= marketpanel_siteoptions_uploader_function($value['id'],$value['std'],null);
		...
		case "upload_min":
			...
			$output .= marketpanel_siteoptions_uploader_function($value['id'],$value['std'],'min');
...
function marketpanel_siteoptions_uploader_function($id,$std,$mod){
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">آپلود عکس</span>';
	
	if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
	
	$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">پاک کردن</span>';
	$uploader .='</div>' . "\n";
